...
# Global flag that enables motion when the trigger phrase is detected
motion_active = False

# Define your trigger phrase here (case insensitive)
TRIGGER_PHRASE = "hello"

def voice_listener():
    """
    Listens continuously for audio input using sounddevice and Vosk.
    When the trigger phrase is detected, sets motion_active to True.
    """
    ...
    with sd.RawInputStream(samplerate=16000, blocksize=8000, dtype='int16', channels=1) as stream:
        print("Voice listener started, say the trigger phrase to activate motion.")
        while True:
            data, overflowed = stream.read(4000)
            if len(data) == 0:
                continue
            # Convert the data to bytes before passing it to AcceptWaveform
...
class Motion:
    ...
    def control_arm(self):
        """Main control loop for robotic arm operations. Waits for voice trigger before executing motions."""
        global motion_active
        while True:
            # Wait until the trigger phrase has been spoken
            ...
            # Reset the motion flag so that the arm only acts when the phrase is spoken again
            motion_active = False
...
if __name__ == "__main__":
    ...
    # Create threads for perception, motion, and voice activation
