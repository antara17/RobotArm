...
class Perception:
    ...
    def process_frame(self, frame):
        ...
        if self.detections:
            for (color, contour, area) in self.detections:
                if area > self.min_contour_area:
                    ...
                    # (Optional) Update position and history based on one of the detections.
                    # For example, here we update for the first detection only:
            ...
            # Determine consensus color if history is used (optional)
        ...
        # Display current detected color (if any detection is active)
        cv2.putText(frame, f'Color: {self.current_detected_color}', 
                   (10, height - 10), cv2.FONT_HERSHEY_SIMPLEX, 
                   0.65, self.display_color, 2)
        
        # Reset detections list for next frame
        self.detections = []
        # Also clear the temporary single-detection variables so that next frame they get updated.
        self.detected_contour = None
        self.detected_contour_area = 0
        self.detected_color = None
    ...
    def detect_color_objects(self, lab_image):
        ...
        # Reset the detections list
        self.detections = []
        # Loop over all colors defined in the color range
        for color in self.color_range:
            if color in self.target_colors:
                ...
                for cnt in contours:
                    ...
                    if area > self.min_contour_area:
                        # Save detection as (color, contour, area)
